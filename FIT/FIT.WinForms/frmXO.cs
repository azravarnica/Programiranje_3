...
namespace FIT.WinForms
{
    public partial class frmXO : Form
    {
        ...
        private void RegistrujPotez(object sender)
        {
            if (sender is Button)
            {
                ...
                if (button.Text == "")
                {
                    ...
                    if (KrajIgre())
                    {
                        var pobjednik = (Brojac % 2 == 0) ? ImeIgraca1 : ImeIgraca2;
                        MessageBox.Show("Pobjedio je igrac: " + pobjednik);
                        PostaviStanjeDugmica(false);
                        lblNaredniIgrac.Text = "";
                    }
                    else
                    {

                        Brojac++;
                        lblNaredniIgrac.Text = (Brojac % 2 == 0) ? "Igrac na redu: " + ImeIgraca1 : "Igrac na redu: " + ImeIgraca2;
                    }
                }
                else
        ...
        private void PostaviStanjeDugmica(bool omogucen, bool promijeniText = false, bool promijeniPozadinu = false)
        {
            foreach (var kontrola in Controls)
            {
                if (kontrola is Button && kontrola != btnRestart)
                {
        ...
        private void PripremiIgru()
        {
            ...
            lblNaredniIgrac.Text = "Igrac na redu: " + ImeIgraca1;
        }
